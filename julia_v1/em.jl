...
function em(data,subs,X,betas,sigma::Vector,likfun; emtol=1e-4, parallel=false, startx = [], full=false, maxiter=100, quiet=false)
	if full
		return em(data,subs,X,betas,diagm(0 => sigma),likfun; emtol=emtol, parallel=parallel, startx = startx, maxiter=maxiter, full=full, quiet=quiet)
	else
		return em(data,subs,X,betas,Diagonal(diagm(0 => sigma)),likfun; emtol=emtol, parallel=parallel, startx = startx, maxiter=maxiter, full=full, quiet=quiet)
	end
end
...
function estep(data,subs,startx,X,betas,sigma,likfun; parallel=false)
	...
	if nusub < nsub
		subsfit = uniquesubs
		mus = mus[:,[findall(subs .== uniquesubs[i])[1] for i in 1:nusub]]
		startx = startx[:,[findall(subs .== uniquesubs[i])[1] for i in 1:nusub]]
	else
		subsfit = subs
	end
	...
	if (parallel)
        ...
        # i think this should now be @sync @distributed
		@sync @distributed for i = 1:nusub
	...
	if (nusub < nsub)
		...
		indices = [findall(uniquesubs .== subs[i])[1] for i in 1:nsub]
		h = h[:,:,indices]
		l = l[indices]
		x = x[:,indices]
	end
...
function mstep(x,X,h,sigma;full=false)
	...
	if (~full || isdiag(X[:,:,1]))
		...
		newsigma = (x-mu)*(x-mu)'/nsub + dropdims(mean(h,dims=3), dims=3)
		if (~full)
		...
		while (true)
			...
			mu = computemeans(X,betas)
			newsigma = (x-mu)*(x-mu)'/nsub + dropdims(mean(h,3),dims=3)
...
function embscovmtx(data,subs,x,X,h,betas,sigma,likfun; parallel=false, full=false, nsamples=100, emtol=1e-3, quiet=true)
